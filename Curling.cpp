...
int _runtimeTerror_()
{
    ...
    for(int i=0;i<n;++i) {
        cin >> a[i].F >> a[i].S;
        amin(rc, a[i].F * a[i].F + a[i].S * a[i].S);
    }
    ll yc = INF;
    int m;
    cin >> m;
    ...
    for(int i=0;i<m;++i) {
        cin >> b[i].F >> b[i].S;
        amin(yc, b[i].F * b[i].F + b[i].S * b[i].S);
    }
    for(int i=0;i<n;++i) {
        ll dis = a[i].F * a[i].F + a[i].S * a[i].S;
        if(dis <= (rs + rh) * (rs + rh) and dis <= yc) {
            ++r;
        }
    }
    for(int i=0;i<m;++i) {
        ll dis = b[i].F * b[i].F + b[i].S * b[i].S;
        if(dis <= (rs + rh) * (rs + rh) and dis <= rc) {
            ++y;
        }
    }
    cout << r << " " << y << "\n";
